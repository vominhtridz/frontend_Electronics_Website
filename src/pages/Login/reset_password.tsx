...
export const Reset_password = () => {
  ...
  const codeverify = Cookies.get("code")
  ...
  const handleResetPassword = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await axios.post(SummaryApi.reset_password.url, data, {
        headers: {
  ...
  useEffect(() => {
    if (!codeverify || codeverify != code) {
      navigate("/login") // Chuyển hướng nếu không có code hoặc code không đúng
    }
  }, [codeverify, code, navigate])
        ...
        <div className='w-full p-6 bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md  sm:p-8'>
          <form action='#' onSubmit={handleResetPassword} method='post'>
            <div className='relative'>
              <label className='text-gray-800 text-sm font-medium mb-2 block pt-1'>Mật khẩu</label>
              <input
                required
                value={password}
                id='password'
              ...
              />
              <div className='cursor-pointer text-xl absolute top-11 right-4' onClick={ShowPwd}>
            ...
            <div className='relative'>
              ...
              <input
                ...
                placeholder='Nhập Mật khẩu'
              />
              <div className='cursor-pointer text-xl absolute top-11 right-4' onClick={ShowPwd1}>
